...
export const ResumeTable = ({ resume }: { resume: Resume }) => {
  ...
  const workExperiences = Array.isArray(resume.workExperiences) && resume.workExperiences.length > 0
    ...
    : [];
  const skills = Array.isArray(resume.skills?.descriptions) ? [...resume.skills.descriptions] : [];
  const featuredSkills = Array.isArray(resume.skills?.featuredSkills)
    ? resume.skills.featuredSkills.filter((item) => item.skill && item.skill.trim()).map((item) => item.skill).join(", ").trim()
    : "";
  if (featuredSkills) {
